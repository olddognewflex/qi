...
- `internal/config/` — TOML loader with env var overrides. Derives `InboxPath`, `NotesPath`, `DailyPath` from `VaultPath` (subdirs `00-inbox`, `20-notes`, `30-daily`). `TaskFilePath` defaults to `10-tasks/inbox.md` relative to vault. Also parses `[[mcp_servers]]` (external MCP servers qid connects to), the `[ai]` section (provider/model defaults for `qi ai run`), `[[project_vault]]` entries (per-project vault/task-file overrides used by cross-vault sync), and the `[launch]` section (default AI harness for `qi launch harness`, with optional per-`[[project_vault]]` `[project_vault.launch]` overrides). `Config.ResolveLaunch(project)` resolves the harness in order **per-project `[project_vault.launch]` > global `[launch]` > `$AI_HARNESS` > `$AI_EDITOR`**; `Config.EffectiveProject(flag)` selects the project from an explicit `--project` flag (strict — typos error) else `$WORK_CONTEXT` (lenient — an unmapped value falls through to the global harness).
