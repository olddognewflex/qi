...
- `internal/commands/` — **thin** Cobra handlers. Wiring only: parse flags, call service, format output. No business logic. New subcommands register in `root.go`. `ai.go` is the client side of qid: `qi ai tools list|call`, `qi ai run`, `qi ai approvals|approve|deny`. `launch.go` is `qi launch harness` (alias `qi launch ai`): resolves an external AI harness from config and runs it with `QI_VAULT_PATH` exported (the project's `vault_path`, else the global vault) and cwd set to the project's `dev_path` if configured (else the current dir is inherited — no chdir) — non-detached harnesses replace the qi process (`syscall.Exec`, unix), detached ones spawn and return. Platform exec split lives in `launch_exec_unix.go` / `launch_exec_windows.go`.
...
- `internal/config/` — TOML loader with env var overrides. Derives `InboxPath`, `NotesPath`, `DailyPath` from `VaultPath` (subdirs `00-inbox`, `20-notes`, `30-daily`). `TaskFilePath` defaults to `10-tasks/inbox.md` relative to vault. Also parses `[[mcp_servers]]` (external MCP servers qid connects to), the `[ai]` section (provider/model defaults for `qi ai run`), `[[project]]` entries (per-project `vault_path`/task-file overrides used by cross-vault sync, plus an optional `dev_path` used as the `qi launch harness` working dir; the `project` key matches a task's first `#tag`), and the `[launch]` section (default AI harness for `qi launch harness`, with optional per-`[[project]]` `[project.launch]` overrides). `Config.ResolveLaunch(project)` resolves the harness in order **per-project `[project.launch]` > global `[launch]` > `$AI_HARNESS` > `$AI_EDITOR`**; `Config.EffectiveProject(flag)` selects the project from an explicit `--project` flag (strict — typos error) else `$WORK_CONTEXT` (lenient — an unmapped value falls through to the global harness).
