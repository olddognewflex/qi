...
- `internal/skills/` — deterministic composed workflows exposed as `SourceSkill` tools: `daily-review`, `process-inbox` (read-only; proposes task/note/archive per `00-inbox/` capture via deterministic heuristics) + `process-inbox-apply` (mutating; executes one proposed action). Chain existing services and read the vault; **never call an LLM, never write silently.** Read-only by default; mutating skills must set `Mutating: true` so policy gates them.
...
4. **Obsidian task-line compatibility.** Don't change `ParseTaskLine`/`FormatTaskLine` output format without verifying round-trip with existing vault files. Project tag becomes the first `#tag`; due date is `📅 YYYY-MM-DD`; recurrence is `🔁 <rule>` (emitted after tags, before `⏳`/`📅`). A foreign `^ref` or unsupported recurrence rule must survive untouched.
